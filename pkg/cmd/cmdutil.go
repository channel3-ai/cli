...
func getDefaultRequestOptions(cmd *cli.Command) []option.RequestOption {
	opts := []option.RequestOption{
		option.WithHeader("User-Agent", fmt.Sprintf("Channel3/CLI %s", Version)),
	...
	if cmd.IsSet("language") {
		opts = append(opts, option.WithLanguage(cmd.String("language")))
	}
	if cmd.IsSet("country") {
		opts = append(opts, option.WithCountry(cmd.String("country")))
	}
