...
var reportingClicksList = cli.Command{
	...
	Action:          handleReportingClicksList,
	HideHelpCommand: true,
}

func handleReportingClicksList(ctx context.Context, cmd *cli.Command) error {
	...
	params := channel3go.ReportingClickListParams{}
	...
	if format == "raw" {
		...
		_, err = client.Reporting.Clicks.List(ctx, params, options...)
		if err != nil {
			return err
		}
		...
		return ShowJSON(obj, ShowJSONOpts{
			...
			Title:          "reporting:clicks list",
			Transform:      transform,
		})
	} else {
		iter := client.Reporting.Clicks.ListAutoPaging(ctx, params, options...)
		maxItems := int64(-1)
